...
myInteractionBargraph = function( D, dv, xv, zv=NULL, main=NULL, main.adj=0, xlab=NULL, xlim=NULL, xnames=NULL, ylab=NULL, ylim=NULL, yat=NULL, percent=FALSE, znames=NULL, zcol=NULL, zpos='top', bar.colors=NULL ) {
    ## calculate margins
    # top margin
    if ( is.null(main) ) {
        top     = 0.75
    ...
    if (is.null(zv)) {
        ...
        x.at    = colMeans(x$xvals)
    }
    par( mgp=x_mgp )
    axis( side=1, at=x.at, labels=xnames )
    title( xlab=xlab )
    ## legend
    if (!is.null(zv)) {
        if (is.null(znames) ) {
...
png( paste0(graphics_path,"Figure 3 SD add ret by params.png"), width=6.5, height=6, units='in', res=R, pointsize=s )
par(mfrow=c(2,2))
...
png( paste0(graphics_path,"Figure 4 FA acc.png"), width=6.5, height=3, units='in', res=R, pointsize=s )
par(mfrow=c(1,2))
plotFAacc('data')
plotFAacc('simu')
dev.off()
...
png( paste0(graphics_path,"Figure 5 DA acc.png"), width=6.5, height=3, units='in', res=R, pointsize=s )
par(mfrow=c(1,2))
plotDAacc('data')
plotDAacc('simu')
dev.off()
...
if (type=="main") {
    ...
    D = adply(D, 1, function(r) {
        ...
        if (detailed) {
            ...
            for (param in c('g', 'd', 'rt_mu', 'ice')) {
                if (param=='g') {
                    fitx = lm(outcome~d+d2+rt_mu+rt_mu2+ice+ice2+predictor, data=X)
                } else if (param=='d') {
                    fitx = lm(outcome~g+g2+rt_mu+rt_mu2+ice+ice2+predictor, data=X)
                } else if (param=='rt_mu') {
                    fitx = lm(outcome~g+g2+d+d2+ice+ice2+predictor, data=X)
                } else if (param=='init_count') {
                    fitx = lm(outcome~g+g2+d+d2+rt_mu+rt_mu2+predictor, data=X)
                }
            r[,paste0(param,'_B')] = summary(fit3)$coefficients[param,'Estimate']
